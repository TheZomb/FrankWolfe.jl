...
colors = ["gold", "yellow", "orange", "purple", "darkorange2", "firebrick3"]
iterates = unique!(iterates)

# use previous iterates to compute a richardson extrapolation
target = [2.0, 0.5]
extrapolates = []
for i in 2:2:length(iterates)
    println("Franke Wolfe original iterate:")
    println(iterates[Int(i / 2)])
    println("$(iterates[i]) \tGap: $(norm(target - iterates[i]))")

    println("Richardson Extrapolate:")
    println((2 * iterates[i])[2])
    explt = 2 * iterates[i] - iterates[Int(i / 2)]
    push!(extrapolates, explt)
    println("$(explt) \tGap: $(norm(target - explt))")
    println("---")
end

println()
println("Target: [2.0, 0.5000005296253868]")

for i in eachindex(iterates)
    col_const = i * 1 / (length(iterates))
    scatter!(
        [iterates[i][1]],
        [iterates[i][2]],
        label=string("x_", i - 1),
        markersize=2,
        color=string("#", hex(RGB(col_const, col_const, col_const))),
    )
end
